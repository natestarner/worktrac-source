...
function PRsTabContent() {
  ...
  return (
    <div>
      ...
      {/* The controls block -- see controlsBlockStyle. */}
      {!loading && prs.length > 0 && (
...
// The controls (Record/Sort, search, tags) sit --space-3 apart as one block, --space-6 above the
// first record (design-system.md's "Vertical rhythm"). Everything on this tab used to share one
// flat 10px flex gap -- label, field, search and row alike -- with the filter bar's own 18px margin
// added only below the search, so the controls ran straight into the board and read as part of it.
...
// One gap between the cards of every list in the app -- see design-system.md's "Vertical rhythm".
const rowListStyle = { display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' };
