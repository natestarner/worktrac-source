...
// shipped carries no `plan` key, so it hydrates as undefined -- and showing "Go Plus" to someone who
...
// The cost is that it cannot distinguish PAST_DUE (which derives to PLUS, deliberately: a household
...
// Plus pill, with Free getting an outline star -- the mark meaning "you have this", the star meaning
// "not yet". That reading is gone: the mark now means Huddle Plus the PRODUCT, so "Go Plus" reads as
// "go Huddle Plus" rather than as a generic upsell, and the same convention holds wherever Plus is
// named as a product (BillingTab's plan heading, PlusCelebration, HistoryWindowModal's benefits
// block). What still signals possession is the PILL, not the glyph -- .plan-badge--plus's fixed
// bright identity colours against this one's transparent outline. See .claude/rules/billing.md.
//
// Labelled "Go Plus", NOT "Upgrade to Plus": a Free household standing on /app/billing has this
// control and that screen's primary button on screen together, and two controls sharing an
// accessible name make every Playwright getByRole(name: 'Upgrade to Plus') a strict-mode violation.
// The two strings are mutually non-containing, per frontend-core.md. ("Upgrade" alone would be
// worse -- it is a substring of "Upgrade to Plus" and would match both.)
//
...
export default function PlanBadge() {
  ...
  if (plan === 'FREE') {
    // No hairline override here, unlike the Plus pill below: .plan-badge--upgrade is
    ...
    //
    // size 14 matches the Plus pill exactly, so the badge does not resize when a household upgrades
    // -- HuddleMark's height is round(size * 115/129), i.e. 12px at 14, the same box the outline
    // star occupied at size 12. HuddleMark is aria-hidden, so it never touches the "Go Plus"
  ...
  // spans, so getByText('Plus', { exact: true }) is unmoved.
  //
  // hairline pinned to the light value: this pill's own background is a fixed light gradient
  // regardless of theme (see .plan-badge--plus's comment), so it is the one caller whose ground
