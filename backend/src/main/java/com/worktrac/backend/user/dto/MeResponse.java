...
//
// `accounts` is every household THIS LOGIN belongs to, including the current one. It exists so
...
//
// A client on an older auth snapshot has no `accounts` at all, which reads as "nowhere to switch
...
public record MeResponse(UserDto user, AccountDto account, MembershipDto membership,
                          List<PersonDto> people, List<AccountChoiceDto> accounts) {
}
