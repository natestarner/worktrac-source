 ...
 *
 * <p><b>Choose a household</b> — {@code token} is null, {@code accounts} lists what this login
 ...
 * fails in the safe direction: an old client that ignores {@code accounts} entirely gets a null
 ...
 * @param accounts     the choices, non-empty only in the choose-a-household shape
...
public record AuthResponse(String token, UserDto user, AccountDto account,
                            ...
                            List<AccountChoiceDto> accounts, String selectionToken) {

    /** The signed-in shape: exactly what every caller returned before accounts could be plural. */
    ...
    public static AuthResponse chooseAccount(UserDto user, List<AccountChoiceDto> accounts,
                                                String selectionToken) {
        return new AuthResponse(null, user, null, null, null, accounts, selectionToken);
    }
}
