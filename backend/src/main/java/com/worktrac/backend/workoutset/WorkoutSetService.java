...
public class WorkoutSetService {
    ...
    // last_activity_at. This is the path a live (not retroactive) set always takes.
    @Transactional
    public LogSetResultDto logLiveSet(Long accountId, Long personId, LogSetRequest request) {
        ...
        return insertSetAndDetectPr(person, session, exercise, request.weight(), request.reps(), account.getDefaultUnit());
    }
    ...
    private LogSetResultDto insertSetAndDetectPr(Person person, WorkoutSession session, Exercise exercise,
                                                  BigDecimal weight, int reps, String unit) {
        Optional<BigDecimal> prevBestComparableLb = statsService.getBestComparableLb(person.getId(), exercise.getId());

        WorkoutSet set = workoutSetRepository.save(new WorkoutSet(session, person, exercise, weight, reps, unit));
