...
test.describe('Offline cache warming — unvisited person/tab', () => {
  test('a newly added person\'s History resolves while offline, even though History was never visited for them', async ({
    ...
    // Wait for the offline banner (and the layout reflow it causes) to settle before navigating --
    // clicking immediately after setOffline can race the banner's insertion above the tab nav.
