...
| Deleting a set whose create hasn't been confirmed | `deleteQueuedSet` — cancels only a create that provably never left the device, otherwise queues a `DELETE_SET` against its tempId | Calling `cancelQueuedWritesForSet` from a delete path — a create already on the wire lands anyway, and the "deleted" set survives on the server (`2026-09-23`) |
...
| Two PR predicates (`log-screen.md`) | Celebration vs. "was this a PR when recorded" | Not a connectivity branch at all, and no longer three: the Log pill's `\|Δ\| < 0.5` tie rule was **retired** (2026-09-20) and every record badge in the app now comes from one fold. The two that remain differ only in what "prior best" means, and share their maths through an exported `SET_MEASURE_VALUE` |
...
| `SessionSummary` remove | The deletes are durable, but the entry point stays `OfflineDisabledWrap`ped whenever the session may hold server rows for the entry (`mayHaveServerRows`: a synced set in the row, **or** a succeeded create `history` may not reflect yet) | Enumerating which rows to delete needs a live `listSessionSets` read. The *write* is no longer the limitation — the *read* is. Gate on the row alone and a just-synced set that `history` hasn't caught up to is never deleted (`2026-09-23`) |
